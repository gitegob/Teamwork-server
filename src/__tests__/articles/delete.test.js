...
describe('Delete article tests', () => {
  ...
  it('DELETE/ user should not delete another\'s article', async (done) => {
    const res = await request(app).delete(`/api/articles/${utils.jonArticleId}`)
      .set('Authorization', `Bearer ${utils.branToken}`);
  ...
  it('POST/ user should flag an article', async (done) => {
    const res = await request(app).post(`/api/articles/${utils.jonArticleId}/flags`)
      .set('Authorization', `Bearer ${utils.branToken}`)
      .send(utils.flagBody);
  ...
  it('DELETE/ user should delete their article', async (done) => {
    const res = await request(app).delete(`/api/articles/${utils.branArticleId}`)
      .set('Authorization', `Bearer ${utils.branToken}`);
    expect(res.status).toEqual(200);
    expect(res.body).toHaveProperty('message');
    done();
  ...
  it('DELETE/ user should not delete a article with invalid id', async (done) => {
    const res = await request(app).delete('/api/articles/brian')
      .set('Authorization', `Bearer ${utils.branToken}`);
